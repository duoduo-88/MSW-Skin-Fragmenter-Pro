...
def overlap_fill_density_blocks(
    ...
    """
    依據主圖alpha==255區域，隨機用block_size、random_range決定尺寸，填充到總像素數近似fill_percent%，
    每片都做，最後用主圖遮罩裁切
    progress_cb: (cur, total, msg) 進度回呼
    """
...
def apply_interfere_masked(orig, interfere, main_mask):
    """
    orig: 原碎片 (np.ndarray, RGBA)
    interfere: 干擾圖 (np.ndarray, RGBA)
    main_mask: bool型遮罩 (alpha==255) (np.ndarray, shape=(H,W))
    return: 只在 main_mask=True 的區域合成干擾像素，其他區域維持 orig
    """
    out = orig.copy()
...
def build_interfere_block_pool(src_arr, block_size=10, random_range=2, pool_size=400):
    """從 src_arr 的 alpha>0 區域隨機取出一批小 patch 作為干擾素材池"""
    h, w = src_arr.shape[:2]
    alpha = src_arr[..., 3]
...
class SplitThread(QtCore.QThread):
    ...
    def run(self):
        try:
            ...
            pad = max(4, self.blocksize // 2)
            sx = max(0, min_x - pad)
            sy = max(0, min_y - pad)
            ex = min(w, max_x + pad + 1)
            ey = min(h, max_y + pad + 1)
...
class MainWindow(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.checkbox_items = []
        self.setWindowTitle("MSW造型防盜拆解工具 MSW Skin Fragmenter 高性能版 v1.0.0")
    ...
    def initUI(self):
        ...
        mask_row.addWidget(QHelpButton("若有遮罩圖，僅遮罩圖中的不透明區域會參與分割，否則僅主圖的透明度判斷。遮罩圖需與主圖同尺寸。"))
    ...
    def abort_split(self):
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            ...
            QtWidgets.QApplication.processEvents()  # 顯示UI狀態
            python = sys.executable
            os.execl(python, python, *sys.argv)
