...
class InterferePanel(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.density.setValue(30)
        self.density_lbl = QtWidgets.QLabel("干擾密度: 30%")
...
class MainWindow(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        # 拆解參數
        self.num_input = QtWidgets.QSpinBox(); self.num_input.setRange(1,10); self.num_input.setValue(8)
    ...
    def _confirm_partial_split_warning(self) -> bool:
        ...
        box.setText(
            "局部分割會把『選取區域』範圍內既有碎片的『干擾像素以及劣化』效果清除。\n\n"
            "建議流程：先完成整體分割拆解以及局部分割後 → 再執行干擾像素/劣化。\n\n"
            "若已經執行『干擾像素以及劣化』後需要局部分割，建議執行『還原初始分割』後再重新一次建議流程。"
        )
        box.setStandardButtons(QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
        box.setDefaultButton(QtWidgets.QMessageBox.No)

        cb = QtWidgets.QCheckBox("下次重啓前不再顯示")
        box.setCheckBox(cb)
    ...
    def partial_split(self):
        ...
        mask_crop = None
        if self.mask_img is not None:
            mask_crop = self.mask_img[y:y+h, x:x+w]

        # 3) 組合 SplitThread 參數（沿用目前 UI）
        num = self.num_input.value()
        blocksize = self.block_input.value()
        rand_factor = self.rand_input.value()
        strict_mask = True  # 讓分配時優先覆蓋有有效像素區
        if hasattr(self, "mask_crop_cb"):
            strict_mask = self.mask_crop_cb.isChecked()

        # 4) 在小圖上分割（同步跑：直接重用 SplitThread 的 run() 內容邏輯較重；改為簡化 inline）
        th = SplitThread(np2pil(main_crop), np2pil(mask_crop) if mask_crop is not None else None,
                         num, blocksize, rand_factor, strict_mask=strict_mask)
        result_holder = {"imgs": None}
        def _cap(imgs): result_holder["imgs"] = imgs
        th.result.connect(_cap)
        th.run()
        new_small_imgs = result_holder["imgs"]
        if not new_small_imgs:
            QtWidgets.QMessageBox.warning(self, "局部分割失敗", "選定區域內沒有可分割的像素")
            return

        # 5) 若有勾「不溢出」，一次套遮罩 alpha（局部）
        if hasattr(self, "mask_crop_cb") and self.mask_crop_cb.isChecked():
            cover_mask_crop = None
            if self.mask_img is not None:
                cover_mask_crop = (self.mask_img[...,3] > 0)[y:y+h, x:x+w]
            else:
                cover_mask_crop = (self.main_img[...,3] > 0)[y:y+h, x:x+w]
            tmp = []
            for arr in new_small_imgs:
                ...
                mask_full[y:y+h, x:x+w] = cover_mask_crop
                tmp.append(crop_to_primary_mask(big, mask_full))
            new_imgs_full = tmp
        else:
            # 直接放大（貼回原座標）
            new_imgs_full = []
        ...
        for name in list(self.fragment_order):
            ...
            if img is None:
                continue
            img[y:y+h, x:x+w, 3] = 0  # 直接把該區域 alpha 清空
        ...
        try:
            ...
            print("局部分割後同步失敗", e)
        self.set_status(f"局部分割完成：新增 {max(0, newly - existed)}，共 {len(self.fragment_order)} 片", True)
        if self.fragment_order:
            self.fragment_list.setCurrentRow(0)
            cur = self.fragment_data[self.fragment_order[0]]
            self.img_wrap.preview.set_image(cur)
