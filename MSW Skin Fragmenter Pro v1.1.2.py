...
def simple_block_degrade(src_img, block_size, rand_range, density, noise_strength, brightness_strength, color_strength):
    ...
    # 計算要放幾個 block：以 area * density / block_area 為基準
    ...
    for _ in range(n_blocks):
        ...
        patch = out[y0:y0+sz, x0:x0+sz, :3]  # RGB as float
        ...
        # 加 noise（高斯）
        sigma = (noise_strength / 100.0) * 30  # 可調整 scale
...
def gen_multi_overlap_interfere(
    ...
    main_img_arr=None,  # 需提供原圖給二次build pool
    block_size=10,      # 記得傳進來主block_size
    random_range=2,     # 傳進來隨機度
    fill_stages=[0.92, 0.97],  # 主填/細補階段覆蓋率
    patch_min_size_ratio=0.4   # 細補block為主block多少倍（0.4=40%）
):
...
class SplitThread(QtCore.QThread):
    ...
    def run(self):
        try:
            ...
            valid_coords_arr = np.argwhere(primary_mask)  # array of [y, x]
...
class OverlapThread(QThread):
    ...
    def run(self):
        ...
        self._executor = None
        try:
            # 若 apply_overlap_to_all_fragments_mp 可接受 executor，可這樣傳
            # 否則你可以在這裡進行多進程調用（和上面 InterfereGenThread 一樣）
            # 這裡假設你還沒多進程（保留設計，未來好插進去）
