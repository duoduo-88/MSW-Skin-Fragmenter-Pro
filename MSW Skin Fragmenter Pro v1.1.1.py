...
def gen_multi_overlap_interfere(
    ...
    block_size=10,      
    random_range=2,    
    fill_stages=[0.92, 0.97],  
    patch_min_size_ratio=0.4   
):
    ...
    fill_targets = [int(total_valid * r) for r in fill_stages]

    max_fill_main = fill_targets[0]
    fail_count = 0
    fail_limit = 4000
    while pasted < max_fill_main and tries < target_pixels * max_try:
        patch = random.choice(block_pool)
        bh, bw = patch.shape[:2]
        y = random.randint(0, h - bh) if h > bh else 0
        x = random.randint(0, w - bw) if w > bw else 0
        ...
        pasted += np.sum((patch[..., 3] > 0) & (~pasted_mask[y:y + bh, x:x + bw]))
        pasted_mask[y:y + bh, x:x + bw] |= (patch[..., 3] > 0)
        tries += 1

    if pasted < fill_targets[1] and main_img_arr is not None:
        small_block = max(2, int(block_size * patch_min_size_ratio))
        small_pool = build_interfere_block_pool(
            main_img_arr,
            block_size=small_block,
            random_range=max(1, int(random_range / 1.2)),
            pool_size=300,
            restrict_mask=main_alpha255_mask
        )
        fail_count = 0
        fail_limit = 2500
        tries2 = 0
        while pasted < fill_targets[1] and tries2 < target_pixels * max_try:
            patch = random.choice(small_pool)
            bh, bw = patch.shape[:2]
            y = random.randint(0, h - bh) if h > bh else 0
            x = random.randint(0, w - bw) if w > bw else 0
            if not allow_overlap and np.any(pasted_mask[y:y + bh, x:x + bw] & (patch[..., 3] > 0)):
...
def gen_interfere_worker(args):
    ...
    interfere = gen_multi_overlap_interfere(
        ...
        main_alpha255_mask=primary_mask
    )
...
class ImagePreview(QtWidgets.QLabel):
    ...
    def paintEvent(self, ev):
        ...
        if self.qimg:
            ...
            pen_color = QtGui.QColor(0, 0, 0) if BG_OPTIONS[self.bg_idx][0] == "check" else QtGui.QColor(255, 255, 255)
        ...
        try:
            ...
            if self.mask_img is not None:
                ...
                if mask_rgba.shape[:2] != (h, w):
                    self.result.emit([])
                    return
                primary_mask = mask_rgba[..., 3] > 0
            else:
                primary_mask = rgba[..., 3] > 0

            valid_coords_arr = np.argwhere(primary_mask)
            if valid_coords_arr.size == 0:
                self.result.emit([])
                return
            valid_coords_set = set((int(x), int(y)) for y, x in valid_coords_arr)
...
class DegradePanel(QtWidgets.QWidget):
    ...
    def on_import(self):
        fn, _ = QtWidgets.QFileDialog.getOpenFileName(self, "匯入劣化原始圖", "", "PNG 圖檔 (*.png)")
        if not fn:
            return
        try:
            from PIL import Image
            ...
            arr = pil2np(im)
            self.loaded_img = arr
            self.orig_preview.set_image(arr)
            self.degraded_img = None
            self.result_preview.set_image(None)
            self.apply_export_btn.setEnabled(False)
        except Exception as e:
            QtWidgets.QMessageBox.warning(self, "匯入失敗", f"載入圖像失敗: {e}")

    def on_generate(self):
        if self.loaded_img is None:
            QtWidgets.QMessageBox.warning(self, "錯誤", "請先匯入要處理的圖像")
            return
        bs = self.block_size.value()
        rr = self.rand_range.value()
        ...
        try:
            ...
            self.degraded_img = degraded
            self.result_preview.set_image(degraded)
            self.apply_export_btn.setEnabled(True)
        except Exception as e:
            QtWidgets.QMessageBox.warning(self, "處理失敗", f"劣化運算錯誤: {e}")

    def on_apply_export(self):
        if self.degraded_img is None:
            QtWidgets.QMessageBox.information(self, "尚未產生", "請先產生劣化圖像")
            return
        fn, _ = QtWidgets.QFileDialog.getSaveFileName(self, "儲存劣化圖", "degraded.png", "PNG 圖檔 (*.png)")
        if not fn:
            return
        try:
            np2pil(self.degraded_img).save(fn)
            QtWidgets.QMessageBox.information(self, "完成", f"已儲存到 {os.path.basename(fn)}")
        except Exception as e:
            QtWidgets.QMessageBox.warning(self, "儲存失敗", f"無法儲存: {e}")
...
class MainWindow(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.checkbox_items = []
        self.setWindowTitle("MSW造型防盜拆解工具 專業版 MSW Skin Fragmenter Pro v1.1.1")
        ...
        self._last_degrade_warning = 0.0 
    ...
    def initUI(self):
        ...
        self.fragment_list.setContextMenuPolicy(QtCore.Qt.NoContextMenu)
        ...
        self.degrade_tab = degrade_proc_tab 
    ...
    def split(self):
        ...
        self.split_thread = SplitThread(pil_main, pil_mask, n, block_sz, rand_sz)
...
def apply_overlap_to_all_fragments_mp(fragments, main_img, mask_img, fill_percent, block_size, random_range, progress_cb=None, abort_cb=None):
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for fut in as_completed(futures):
            ...
            done_cnt += 1
            if progress_cb:
                progress_cb(done_cnt, N, f"重疊像素合成...")
    return results
